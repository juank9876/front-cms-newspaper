...
import { fetchPermalink } from '@/api-fetcher/fetcher'


export async function CardPostCategory({ post, category, isImgBig = false, hideImage = false }: { post: Post, category: Category, isImgBig?: boolean, hideImage?: boolean }) {
  //const categoryUrl = category.parent_id ? category.parent_slug + "/" + category.slug : category.slug
  const res = await fetchPermalink(post.id, "post")
  const postUrl = res.permalink
  ...
  return (
    <>
      ...
      <div className={`group relative hidden ${cardWidth} overflow-hidden bg-white p-0 transition-all duration-200 lg:flex flex-col`}>
        <Link href={`/categories${postUrl}`} className="h-full w-full flex flex-col">
          {/* Imagen */}
          {!hideImage && (
...
export function CardPostRow({ post, category }: { post: Post, category: Category }) {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full border-b last:border-b-0 py-6 gap-4">
      <Link href={`/categories/${categoryUrl}/${post.slug}`} className="flex flex-col md:flex-row w-full border-b last:border-b-0 py-6 gap-4">
        {/* Imagen */}
        ...
        <div className="flex flex-1 flex-col justify-between min-w-0">
          ...
          <h2 className="mb-1 line-clamp-2 text-xl md:text-2xl font-serif font-bold text-slate-900 hover:underline hover:decoration-2 transition-all text-left cursor-pointer">{post.title}</h2>
